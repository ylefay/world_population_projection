...
def decrement(fertility_rate, delta_c):
    cond1 = fertility_rate >= 1.0
    triangle_1c, triangle_2c, triangle_3c, triangle_4c, d_c = delta_c
    triangle_sum = triangle_1c + triangle_2c + triangle_3c + triangle_4c
    cst = -2 * np.log(9)
    _ = (1 / (1 + np.exp(cst * (fertility_rate - triangle_4c - 0.5 * triangle_3c) / triangle_3c)) - \
         1 / (1 + np.exp(cst * (fertility_rate - triangle_sum + 0.5 * triangle_1c) / triangle_1c)))
    return cond1 * _ * d_c
...
def fun_sigma(c1975, sigma0, S, a, b, tau_c, s_tausq, t, fertility_rate):
    ...
    _ = -a * cond1 + b * cond2
    return np.abs(c_1975t * (sigma0 + (fertility_rate - S) * _))
...
class fertility_II:
    ...
    def run(self):
        ...
        if not self.phaseIII:
            sigma = fun_sigma(self.c1975, self.sigma0, self.S, self.a, self.b, self.tau_c, self.s_tausq,
