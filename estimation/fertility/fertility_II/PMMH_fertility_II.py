...
default_prior = {  # First defining the prior on the world-level parameters
    ...
    'delta_4': dists.InvGamma(a=1.0, b=fertility_II_config['world']['delta0'] ** 2),
    'delta_3': dists.InvGamma(a=1.0, b=fertility_II_config['world']['delta0'] ** 2),
    'delta_2': dists.InvGamma(a=1.0, b=fertility_II_config['world']['delta0'] ** 2),
    'delta_1': dists.InvGamma(a=1.0, b=fertility_II_config['world']['delta0'] ** 2),
    ...
    'alpha_3': dists.Normal(loc=fertility_II_config['world']['alpha03'],
                            scale=fertility_II_config['world']['delta0']),
    'psi2': dists.InvGamma(a=1.0, b=fertility_II_config['world']['psi0'] ** 2),
    ...
    'gamma1_c': dists.Cond(lambda theta: dists.Normal(loc=theta['alpha_1'], scale=theta['delta_1'])),
    'gamma2_c': dists.Cond(lambda theta: dists.Normal(loc=theta['alpha_1'], scale=theta['delta_1'])),
    'gamma3_c': dists.Cond(lambda theta: dists.Normal(loc=theta['alpha_1'], scale=theta['delta_1'])),
    'd_c_star': dists.Cond(lambda theta: dists.Normal(loc=theta['chi'], scale=theta['psi2'] ** 0.5)),
    'triangle_4c_star': dists.Cond(lambda theta: dists.Normal(loc=theta['triangle_4'], scale=theta['delta_4'] ** 0.5)),
}
...
def get_tau_c(data):
    ...
    for t in range(len(data)):
        ...
        if _[t] >= 0.5 and local_maximum > 5.5:
            _[t] = -np.inf
    return np.argmax(_)
